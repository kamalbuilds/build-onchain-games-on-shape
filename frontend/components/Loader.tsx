...
const Loader: React.FC<Props> = () => {
  ...
  // Handle if there's no model need to be loaded.
  useEffect(() => {
  ...
  // Close loader if progress finished.
  useEffect(() => {
    if (isFinished) {
      setIsActive(false);
    }
      ...
      {isActive && (
        <motion.div
          className={classNames(
            "absolute top-0 left-0 w-full h-screen z-20 flex justify-center items-center",
            {
